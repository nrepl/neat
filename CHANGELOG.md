...
- `neat-list-connections` opens a `*neat-connections*` tabulated-list buffer with one row per live `neat-connection`: host:port, session prefix, status, default marker. RET sets the connection on the current line as the default; `d` disconnects it; `g` reloads. The buffer auto-refreshes when connections come and go (via `neat-disconnect-functions`).
- `neat-disconnect-connection` interactive command — pick a live connection via `completing-read` and disconnect it. For the library-level primitive, see `neat-disconnect`.
- `neat-show-doc-at-point` (bound to `C-c C-d C-d` in both `neat-mode` and `neat-repl-mode`) pops a `*neat-doc*` help buffer with the symbol's full docstring, arglists, and source location. Eldoc keeps doing its inline summary; this is the full read.
